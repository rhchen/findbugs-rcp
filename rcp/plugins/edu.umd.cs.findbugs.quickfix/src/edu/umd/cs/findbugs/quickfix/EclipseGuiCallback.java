...
public class EclipseGuiCallback implements IGuiCallback {
    ...
    public void registerCloud(Project project, BugCollection collection, final Cloud cloud) {
        cloudListener = new CloudListener() {
            public void statusUpdated() {
                ...
                final IWorkbenchWindow win = QuickfixPlugin.getActiveWorkbenchWindow();
                win.getShell().getDisplay().asyncExec(new Runnable() {
                    public void run() {
                        ...
                        if (part instanceof IEditorPart) {
                            ...
                            Image image = QuickfixPlugin.getDefault().getImageRegistry().get("buggy-tiny-gray.png");
            ...
            public void taskStarted(final CloudTask task) {
                ...
                Job job = new Job(task.getName()) {
                    ...
                    public boolean belongsTo(Object family) {
                        return QuickfixPlugin.class == family;
                    }
    ...
    public String showQuestionDialog(String message, String title, final String defaultValue) {
        ...
        MessageDialog dlg = new MessageDialog(QuickfixPlugin.getShell(), title, null, message, MessageDialog.QUESTION,
    ...
    public void showMessageDialogAndWait(String message) throws InterruptedException {
        MessageDialog.openInformation(QuickfixPlugin.getShell(), null, message);
    }

    public void showMessageDialog(final String message) {
    	QuickfixPlugin.getShell().getDisplay().asyncExec(new Runnable() {
            public void run() {
                MessageDialog.openInformation(QuickfixPlugin.getShell(), null, message);
            }
        });
    ...
    public int showConfirmDialog(final String message, final String title, final String ok, final String cancel) {
        ...
        QuickfixPlugin.getShell().getDisplay().syncExec(new Runnable() {
            public void run() {
                MessageDialog dialog = new MessageDialog(QuickfixPlugin.getShell(), title, null, message, MessageDialog.NONE,
    ...
    public void invokeInGUIThread(Runnable r) {
    	QuickfixPlugin.getShell().getDisplay().asyncExec(r);
    }
    ...
    public void displayNonmodelMessage(final String title, final String message) {
        invokeInGUIThread(new Runnable() {
            public void run() {
                MessageDialog.openInformation(QuickfixPlugin.getShell(), title, message);
            }
        });
    }
