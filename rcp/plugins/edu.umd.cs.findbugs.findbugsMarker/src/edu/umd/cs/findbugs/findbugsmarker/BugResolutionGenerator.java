...
public class BugResolutionGenerator implements IMarkerResolutionGenerator2 {
    ...
    public IMarkerResolution[] getResolutions(IMarker marker) {
        try {
            ...
            BugResolutionAssociations resolutions = FindbugsMarkerPlugin.getDefault().getBugResolutions();
            if (resolutions == null) {
                return new IMarkerResolution[0];
            }
            return resolutions.getBugResolutions(type);
        } catch (CoreException e) {
        	QuickfixPlugin.getDefault().logException(e, "Marker has no FindBugs bug-type.");
            return null;
        }
    }
